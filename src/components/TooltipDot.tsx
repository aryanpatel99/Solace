...
export default function TooltipDot({ status, dateLabel }: TooltipDotProps) {
    ...
    if (status === "passed") {
        color = "bg-stone-300 dark:bg-zinc-400";
    } else if (status === "today") {
        color = "bg-stone-700 dark:bg-white scale-125 shadow-[0_0_8px_rgba(0,0,0,0.15)] dark:shadow-[0_0_8px_rgba(255,255,255,0.3)] animate-pulse";
    } else {
        color = "bg-stone-100 border border-stone-200 dark:bg-zinc-900 dark:border-zinc-800/60";
    }
    ...
    return (
        <div className="relative group">
            ...
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1
                            ...
                            rounded-full opacity-0 group-hover:opacity-100 transition pointer-events-none whitespace-nowrap">
                {dateLabel}
            </div>
        </div>
    );
